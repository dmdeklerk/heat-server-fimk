...
class RsAddress {
    ...
    guess_errors() {
        let el = 0, b = [0, 0, 0, 0, 0], t = [];
        let deg_lambda = 0, lambda = [1, 0, 0, 0, 0];
        for (let r = 0; r < 4; r++) {
            let discr = this.calc_discrepancy(lambda, r + 1);
            if (discr != 0) {
                deg_lambda = 0;
                for (let i = 0; i < 5; i++) {
        ...
        for (let r = 0; r < errors; r++) {
            ...
            if (t) {
                let denom = this.gmult(lambda[1], 1) ^
                    this.gmult(lambda[3], gexp[(root * 2) % 31]);
                if (denom == 0)
                    return false;
                if (pos > 12)
                    pos -= 14;
    ...
    add_guess() {
        let s = this.toString(), len = this.guess.length;
        if (len > 2)
            return;
        for (let i = 0; i < len; i++) {
            if (this.guess[i] == s)
    ...
    from_acc(acc) {
        let inp = [], out = [], pos = 0, len = acc.length;
...
//# sourceMappingURL=rs-address.js.map